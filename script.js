...
for(let i = 0; i < stations.length; i += 1){
    let threeLetters = stations[i].slice(0, 3);
    let index = stations[i].indexOf(';');
    let forReading = stations[i].slice(index + 1);
    let wholeString = threeLetters + forReading;
...
function verifyCats(catsJane, catsJulia){
  ...
  let allCats = newCatsJane.concat(catsJulia);
...
let strings1 = ['yes', 'hello', 'no', 'easycode', 'what'];
function stringFilter(arr){
  let newStrings = strings1.filter(function(item){
    return item.length > 3;
  })
  return newStrings;
}
console.log(stringFilter(strings1));
...
function getAverageHumanAge(catAges){
  const humanAges = catAges.map(function(item){
    if(item <= 2){
      return item * 10;
    }
    else if(item > 2){
  ...
  const newHumanAges = humanAges.filter(function(item){
    return item > 10;
  });
  const sum = newHumanAges.reduce(function(acc, item, arr){
